...
func sessionWrapper(ws *websocket.Conn, m *sync.Mutex) {
	...
	timeLeft := func(sec float64) string {
	...
	launchSend := func(ws *websocket.Conn, m *sync.Mutex) {
			timer := "25:00"
	...
	clientSend := func(ws *websocket.Conn, m *sync.Mutex) {
		start := time.Now()
		for {
	...
	// Wait for timer
	waitTimer := func(ws *websocket.Conn, m *sync.Mutex, timer *time.Timer) {
			for {
	...
	serverRecieve := func(ws *websocket.Conn, m *sync.Mutex) {
		for {
			// sleep
			time.Sleep(100 * time.Millisecond)
			// read
